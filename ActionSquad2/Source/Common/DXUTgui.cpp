...
{   
	...
	if (!m_bMinimized)
	{
		...
		if (m_Textures.GetSize() > 1)
		{
			DXUT_SCREEN_VERTEX vertices[4] =
			{
				// the texture coord was start from 0.5, 0.5
				(float)m_x,           (float)m_y,            0.5f, 1.0f, m_colorTopLeft, 0.0f, 0.0f, 
				(float)m_x + m_width, (float)m_y,            0.5f, 1.0f, m_colorTopRight, 0.89f, 0.0f,
				(float)m_x + m_width, (float)m_y + m_height, 0.5f, 1.0f, m_colorBottomRight, 0.89f, 0.13f, 
				(float)m_x,           (float)m_y + m_height, 0.5f, 1.0f, m_colorBottomLeft, 0.0f, 0.13f, 
			};
			...
			DXUTTextureNode *pTextureNode = GetTexture(1);
			//pd3dDevice->SetMaterial(&g_mtrWhite);
...
bool CDXUTDialog::MsgProc(HWND /*hWnd*/, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_bCaption)
	{
		...
		else if (uMsg == WM_LBUTTONUP && bDrag)
		{
			POINT mousePoint = { short(LOWORD(lParam)), short(HIWORD(lParam)) };
...
void CDXUTDialog::InitDefaultElements()
{
	...
	GetExePathW(texPath, ARRAY_SIZE(texPath));
	StringCchPrintf(texPath, MAX_PATH, L"%sControlsEd\\dxutcontrols.dds", texPath);
	SetTexture(0, texPath);
	SetFont(0, L"Arial", 13, FW_NORMAL);

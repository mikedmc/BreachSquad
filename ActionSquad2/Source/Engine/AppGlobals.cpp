...
void App_LoadUserData()
{
	...
	if (arrVersionData[0] == 514) //old supported version RC5
	{
		//alert me
		LOG( L"[Warning] LoadUserData: Older version found! Converting from %d to %d", arrVersionData[0], _VERSION_DATAFILE_);
		//--- persistent memory ---
		OS_fread(g_userData, sizeof(int), K_MEMID_SLOTS_COUNT, fl);
		//--- level stats have different format ---
		CLevelStats_v514 g_oldlevelStats[K_GAME_LEVELS_PER_CHAPTER * K_GAME_MAX_CHAPTERS];
		OS_fread(g_oldlevelStats, sizeof(CLevelStats_v514), ARRAY_SIZE(g_oldlevelStats), fl);
		//translate to new
		for (int kk = 0; kk < K_GAME_LEVELS_PER_CHAPTER * K_GAME_MAX_CHAPTERS; kk++)
		{
			g_levelStats[kk].nLevelType = g_oldlevelStats[kk].nLevelType;
			g_levelStats[kk].nStars = g_oldlevelStats[kk].nStars;
			g_levelStats[kk].nPlayedTimes = g_oldlevelStats[kk].nPlayedTimes;
			g_levelStats[kk].nScoreSolo = g_oldlevelStats[kk].nXPpoints;
			g_levelStats[kk].nBestTimeSec_Solo = g_oldlevelStats[kk].nBestTimeSec;
			g_levelStats[kk].nScoreCoop = 0;
			g_levelStats[kk].nBestTimeSec_Coop = 0;
		}
		//--- weapon shop ---
#define K_MAX_WEAPONS_IN_SHOP 30
		UINT32 weaponShop[2 * K_MAX_WEAPONS_IN_SHOP];
		OS_fread(weaponShop, sizeof(UINT32), 2 * K_MAX_WEAPONS_IN_SHOP, fl);
		//save in new format
		for (int kk = 0; kk < K_MAX_WEAPONS_IN_SHOP; kk++)
		{
			//if we find valid weapon name hash with 0 price it means weapon was already unlocked
			if ((weaponShop[kk * 2] != 0) && (weaponShop[kk * 2 + 1] == 0))
			{
				UTGetShop().UnlockItem(weaponShop[kk * 2], true);
			}
		}
#undef K_MAX_WEAPONS_IN_SHOP
		...
		//update statistics
		App_UpdateLevelStats();
		//update level types if loading older version datafile (probably levels changed too)
		if (arrVersionData[1] != _VERSION_INT_)
			App_ParseAllLevelsForData(false);
		//overwrite with new format now
		App_SaveUserData();
		LOG(L"LoadUserData: Saved updated file OK");

		g_userData[K_MEMID_OFFER_RESET_USER_DATA] = 1;
		return;
	}
	else if (arrVersionData[0] == 515) //old RC6
	{
		//alert me
		LOG(L"[Warning] LoadUserData: Older version found! Converting from %d to %d", arrVersionData[0], _VERSION_DATAFILE_);
		//--- persistent memory ---
...
#undef K_MAX_WEAPONS_IN_SHOP
	...
	else if (arrVersionData[0] == 516) //last early access version (no changes to final release)
	{
		//alert me
		LOG(L"[Warning] LoadUserData - Loading last early access user data format! found %d expecting %d", arrVersionData[0], _VERSION_DATAFILE_);
		bOfferResetData = true;
		//go on and load the actual data
	}
	else if (arrVersionData[0] != _VERSION_DATAFILE_)
	{
		//close file
		OS_fclose(fl);
		//alert me
		LOG( L"[Warning] LoadUserData - Wrong user data version! Reset UserData! found %d expecting %d", arrVersionData[0], _VERSION_DATAFILE_);

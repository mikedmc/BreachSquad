...
void CPlayerSelScr::Update(float dTime)
{
		...
		while (g_pNetwork->Recv(buffer, INetwork::MAX_UNRELIABLE_PACKET_SIZE, &bufferSize))
		{
			if (bufferSize > 0)
			{
				...
				if ((packHead.eMessageType == CNetLock::K_NETMSG_TYPE_COMMAND) && (cmd == CNetLock::K_NETCMD_PLAYER_SELECTION))
				{
					...
					g_stringsMgr.SetString(STR_PEER_STARS_VAL, L"%d", (int)netSel.nPlayerStars);
				}
				else
				{
				...
				if (!bDetailsWndOpen)
				{
					...
					else if (eCommand == K_PSS_COMMAND_SELECT)
					{
						...
						switch (playersel->nCursorPosReal)
						{
							...
							{
								CCtrlLayer* layer = UTGetControlsManager().ShowLayerOnce("LAYER_ID_PLAYER_UPGRADE");
								if (layer)
								{
									int nPlayerType = (int)playersel->eType;
									int nXPPoints = App_GetAvailableXPPoints(playersel->eType);
									//write points to string
									g_stringsMgr.SetString(STR_UNUSED_POINTS_VAL, L"%d", nXPPoints);
									//erase description
									g_stringsMgr.SetString(STR_SELECTED_PERK_DESC_VAL, L" ");
									//portrait
									CControl* ctrl = null;
					...
					if (itemtype != PSS_ITEMCAT_NOT_SET)
					{
						...
						else if (eCommand == K_PSS_COMMAND_SELECT)
						{
							...
							else
							{
								...
								{
									UTGetControlsManager().MessageBoxOK(STR_UNLOCK_ITEM, STR_NOT_ENOUGH_STARS);
								}
								else
								{
									CCtrlLayer* layer = UTGetControlsManager().ShowLayerOnce("LAYER_ID_BUY_WEAPON");
									CControl* ctrl = null;
									if (layer != null)
									{
										//set prices strings
										g_stringsMgr.SetString(STR_TEMP1, L"%d", g_userData[K_MEMID_STARS_TOTAL] - g_userData[K_MEMID_STARS_SPENT]);
										g_stringsMgr.SetString(STR_TEMP2, L"%d", wpnPrice);
	...
	{
		SND_PLAY(SNDIDX_DENIED);
		UTGetControlsManager().ShowLayerOnce("LAYER_ID_QUIT_PLAYERSEL");
	}
...
HRESULT CPlayerSelScr::LoadItems()
{
	...
	for (pugi::xml_node xbar = xbarsnode.first_child(); xbar; xbar = xbar.next_sibling())
	{
		...
		if (!xbar.attribute(L"strID_name").empty())
			nbar->nStrIdx_name = g_stringsMgr.getStrIdx(xbar.attribute(L"strID_name").value());
		if (!xbar.attribute(L"strID_desc").empty())
			nbar->nStrIdx_desc = g_stringsMgr.getStrIdx(xbar.attribute(L"strID_desc").value());
		...
		for (pugi::xml_node xperk = xbar.first_child(); xperk; xperk = xperk.next_sibling())
		{
			...
			if (!xperk.attribute(L"strID_desc").empty())
				perk.nStrIdx_desc = g_stringsMgr.getStrIdx(xperk.attribute(L"strID_desc").value());
	...
	for (pugi::xml_node bclass = rootnode.first_child(); bclass; bclass = bclass.next_sibling())
	{
		...
		if (!bclass.attribute(L"strID_desc").empty())
			itm->nStrIdx_desc = g_stringsMgr.getStrIdx(bclass.attribute(L"strID_desc").value());
		if (!bclass.attribute(L"strID_difficulty").empty())
			itm->nStrIdx_difficulty= g_stringsMgr.getStrIdx(bclass.attribute(L"strID_difficulty").value());
		//load upgrade bars
		//TEAM BARS
		...
		for (pugi::xml_node btype = bclass.first_child(); btype; btype = btype.next_sibling())
		{
			...
			for (pugi::xml_node bnode = btype.first_child(); bnode; bnode = bnode.next_sibling())
			{
				...
				if (!bnode.attribute(L"strID_name").empty())
					itdata.strIdxScreenName = g_stringsMgr.getStrIdx(bnode.attribute(L"strID_name").value());
				...
				if (!bnode.attribute(L"strID_ALTname").empty())
					itdata.strIdxALTscreenName = g_stringsMgr.getStrIdx(bnode.attribute(L"strID_ALTname").value());
				...
				for (int kk = 0; kk < 4; kk++)
				{
					...
					if (!bnode.attribute(strKey).empty())
						itdata.nStatsData[kk * 2] = g_stringsMgr.getStrIdx(bnode.attribute(strKey).value());
				...
				if (!bnode.attribute(L"strID_longDesc").empty())
					itdata.strIdxLongDescription = g_stringsMgr.getStrIdx(bnode.attribute(L"strID_longDesc").value());
...
void CPlayerSelScr::PaintPlayerSelectionWindow(int nPlayerOrdinal, D3DXVECTOR2 pos, float fAlpha, CStringHash * sPlayerName)
{
	...
	DWORD dwWinColor = D3DCOLOR_FFFA(fAlpha);
	...
	{
		dwWinColor = D3DCOLOR_COLORALPHA(0xff333333, fAlpha);
		fAlpha *= 0.4f;
	}
	DWORD dwTextColor = D3DCOLOR_COLORALPHA(K_COLOR_DEFAULT_TEXT, fAlpha);
	DWORD dwTitleColor = D3DCOLOR_COLORALPHA(K_COLOR_WINDOW_TITLE, fAlpha);

	if (playersel->nInstanceID < 0) //empty slot
	{
		RECTXYWH tempbox(winbox.x, winbox.CenterY() - 25, winbox.w, 50);
		CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME3, tempbox, dwWinColor);

		CStringDesc strPlayer;
		if (sPlayerName == null)
			g_stringsMgr.ReplaceTokenInt(&strPlayer, STR_PLAYER_N, 1, nPlayerOrdinal + 1);
		else
			g_stringsMgr.SetStringDesc(&strPlayer, sPlayerName->text);
	...
	else
	{
		...
		if (sPlayerName == null)
			g_stringsMgr.ReplaceTokenInt(&strTemp, STR_PLAYER_N, 1, nPlayerOrdinal + 1);
		else
			g_stringsMgr.SetStringDesc(&strTemp, sPlayerName->text);

		CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_WINDOW2, winbox, dwWinColor);
		//player names
		RECTXYWH clipper;
		...
		if ((!playersel->bSelected) && (playersel->nCursorMoreReal < 0))
		{
			...
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME5, rcCursor, 0xffffffff);
		}
		...
		CtrlMgrDrawHTilingAnim_HeadsOutside(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_INPUT_BOX2, 0, recttemp, dwWinColor);
		recttemp.Set(winbox.x + 44, recttemp.y, winbox.w - 46, 16);
		g_font9b1->DrawString(STR_PLAYER_CLASS_ASSAULTER + (int)playersel->eType, recttemp.CenterX(), recttemp.CenterY(), FONTFLAG_ANCHOR_VCENTERHCENTER, D3DCOLOR_COLORALPHA(K_COLOR_DEFAULT_TEXT_LIGHTER, fAlpha));
		...
		recttemp.h = 32;
		CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME1, recttemp, dwWinColor, -4);
		...
		if (nLevel < K_GAME_MAX_UPGRADE_LEVELS)
		{
			...
			CtrlMgrDrawProgress_HeadsOutside(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_PROGRESS_SM_XP, recttemp2, fPercFill, dwWinColor);
			g_stringsMgr.SetStringDesc(&strDesc, L"%d/%d", nTotalXP - nMinXP, nMaxXP - nMinXP);
			g_font6nc1->DrawString(&strDesc, recttemp2, FONTFLAG_ANCHOR_VCENTERHCENTER, dwWinColor);
		}
		else
		{
			CtrlMgrDrawProgress_HeadsOutside(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_PROGRESS_SM_XP, recttemp2, 1.0f, dwWinColor);
			g_font6nc1->DrawString(STR_MAX_LEVEL, recttemp2, FONTFLAG_ANCHOR_VCENTERHCENTER, dwWinColor);
		}
		//shieldicon and level
		CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.x, recttemp2.CenterY(), ANM_CONTROLS_SPR_PROGRESS_XP_UPGRADE, 9, dwWinColor);
		g_stringsMgr.SetStringDesc(&strDesc, L"%d", nLevel + 1);
		g_font6nc1->DrawString(&strDesc, recttemp2.x - 8, recttemp2.CenterY(), FONTFLAG_ANCHOR_VCENTERHCENTER, D3DCOLOR_COLORALPHA(0xfffdb727, fAlpha));

		//frame nume abilitate
		recttemp2.Set(recttemp.x + 4, recttemp.y + 20, recttemp.w - 8, 8);
		CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME4, recttemp2, dwWinColor);
		...
		{
			CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.x + 4, recttemp2.CenterY(), ANM_CONTROLS_SPR_ICONS_MISC, 0, dwWinColor);
			CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.Right() - 5, recttemp2.CenterY(), ANM_CONTROLS_SPR_ICONS_MISC, 0, dwWinColor);
		}
		...
		CtrlMgrDrawWindow(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_WINDOW1, recttemp, dwWinColor, FONTIDX_8_B1, STR_PRIMARY_WEAPON, dwTextColor);
		//weapon icon
		recttemp2.Set(recttemp.x, recttemp.y + 2, 44, 14);
		CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME6_DARK, recttemp2, dwWinColor);
		...
		else 
		{
			...
			dwWCol = D3DCOLOR_COLORALPHA(0xff888888, fAlpha);
			//paint lock on locked weapons
			CSprite::paintFrame(&m_sprCol, recttemp2.CenterX(), recttemp2.CenterY(), ANM_MENUS_SPR_ICONS_WEAPONS, pItem->iconIdx, dwWCol);
			CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.x + 4, recttemp2.Bottom() - 3, ANM_CONTROLS_SPR_LOCKS, 0, dwWinColor);
			CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.Right() - 4, recttemp2.Bottom() - 3, ANM_CONTROLS_SPR_STARS_2F_SM, 1, dwWinColor);
			//paint cost
			CStringDesc sdPrice;
			g_stringsMgr.SetStringDesc(&sdPrice, L"%d", nWeaponPrice);
		...
		CtrlMgrDrawWindow(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_WINDOW1, recttemp, dwWinColor, FONTIDX_8_B1, STR_GEAR, dwTextColor);
		//icon
		...
		CtrlMgrDrawWindow(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_WINDOW1, recttemp, dwWinColor, FONTIDX_8_B1, STR_ULTIMATE, dwTextColor);
		...
		CtrlMgrDrawHTilingAnim(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_BUTTON1, butframe, recttemp, dwWinColor);
		//textul
		if(!playersel->bSelected)
			recttemp.y -= 1;
	...
	switch (playersel->nCursorPosReal)
	{
		case K_PSS_CURPOS_CLASS:
		{
			...
			for (int kk = 0; kk < nItemsCnt; kk++)
			{
				...
				CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME1, recttemp, dwWinColor);
				//icon
				...
				//shieldicon and level
				CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp.x + 12, recttemp.Bottom() - 5, ANM_CONTROLS_SPR_PROGRESS_XP_UPGRADE, 9, dwWinColor);
				int nxppts = g_userData[K_MEMID_TOTALXP_PER_CLASS_START + kk];
				int nlvllocal = App_GetXPLevel(nxppts);
				g_stringsMgr.SetStringDesc(&strDesc, L"%d", nlvllocal + 1);
				g_font6nc1->DrawString(&strDesc, recttemp.x + 12 - 8, recttemp.Bottom() - 5, FONTFLAG_ANCHOR_VCENTERHCENTER, D3DCOLOR_COLORALPHA(0xfffdb727, fAlpha));
			}

			///--- selected class ---
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME5, tempbox, 0xffffffff);
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME1, localbox, dwWinColor);

			//icon
			recttemp.Set(localbox.x - 2, localbox.y - 2, 38, 38);
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME_WHITE1, recttemp, D3DCOLOR_COLORALPHA(0xff2A3946, fAlpha));
			int nPortraitAnm = (nPlayerOrdinal == 0) ? ANM_MENUS_SPR_CHAR_PORTRAITS_LARGE : ANM_MENUS_SPR_CHAR_PORTRAITS_LARGE_R;
			CSprite::paintFrame(&m_sprCol, recttemp.CenterX(), recttemp.y, nPortraitAnm, (int)playersel->eType, dwWinColor);
			//shieldicon and level
			CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp.x + 14, recttemp.Bottom() - 7, ANM_CONTROLS_SPR_PROGRESS_XP_UPGRADE, 9, dwWinColor);
			g_stringsMgr.SetStringDesc(&strDesc, L"%d", nLevel + 1);
			g_font6nc1->DrawString(&strDesc, recttemp.x + 14 - 8, recttemp.Bottom() - 7, FONTFLAG_ANCHOR_VCENTERHCENTER, D3DCOLOR_COLORALPHA(0xfffdb727, fAlpha));
			//class name
			recttemp2.Set(localbox.x + 48, localbox.y - 1, winbox.w - 62, 16);
			CtrlMgrDrawHTilingAnim_HeadsOutside(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_INPUT_BOX2, 0, recttemp2, dwWinColor);
			g_font9b1->DrawString(STR_PLAYER_CLASS_ASSAULTER + (int)playersel->eType, recttemp2.CenterX(), recttemp2.CenterY(), FONTFLAG_ANCHOR_VCENTERHCENTER, D3DCOLOR_COLORALPHA(K_COLOR_DEFAULT_TEXT_LIGHTER, fAlpha));
			//difficulty and XP
			...
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME_WHITE1, recttemp2, D3DCOLOR_COLORALPHA(0xff1C252E, fAlpha));
			...
			CSprite::paintFrame(&UTGetControlsManager().m_sprCol, tempbox.CenterX(), tempbox.y - off, ANM_CONTROLS_SPR_ARROWS1, K_DIR_UP, dwWinColor);
			//if (playersel->eType < K_PSS_CLASSES_COUNT - 1)
			CSprite::paintFrame(&UTGetControlsManager().m_sprCol, tempbox.CenterX(), tempbox.Bottom() + off, ANM_CONTROLS_SPR_ARROWS1, K_DIR_DOWN, dwWinColor);
		...
		case K_PSS_CURPOS_WEAPON:
		{
			...
			DWORD dwWColDenied = D3DCOLOR_COLORALPHA(0xff887777, fAlpha);
			...
			for (int kk = 0; kk < nItemsCnt; kk++)
			{
				...
				else //locked
				{
					...
					//lock
					CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.x + 2, recttemp2.Bottom() - 3, ANM_CONTROLS_SPR_LOCKS, 3, dwWinColor);
					//name
					recttemp2.Set(recttemp.Right() - 72, recttemp.y - 1, 74, 18);
					g_font6n1->DrawString(pItemSm->strIdxScreenName, recttemp2, FONTFLAG_ANCHOR_TOPRIGHT | FONTFLAG_WRAPTEXT, dwTextColor);
				}
			...
			///--- selected item ---
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME5, tempbox, 0xffffffff);
			CtrlMgrDrawWindow(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_WINDOW1, localbox, dwWinColor, FONTIDX_8_B1, pItem->strIdxScreenName, dwTextColor);
			//weapon icon
			...
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME6_DARK, recttemp, dwWinColor);
			...
			else
			{
				...
				CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.x + 8, recttemp2.Bottom() - 7, ANM_CONTROLS_SPR_LOCKS, 2, dwWinColor);
				CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.Right() - 12, recttemp2.Bottom() - 4, ANM_CONTROLS_SPR_ICONS_MISC, 1, dwWinColor);
				//paint cost
				CStringDesc sdPrice;
				g_stringsMgr.SetStringDesc(&sdPrice, L"%d", nWeaponPrice);
			...
			for (int oo = 0; oo < 4; oo++)
			{
				...
				if (nStrIdx >= 0)
				{
					...
					CtrlMgrDrawProgress_HeadsOutside(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_PROGRESS, recttemp2, fPerc, dwWinColor);
		...
		case K_PSS_CURPOS_GEAR:
		{
			...
			else
			{
				...
				CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.CenterX() - 14, recttemp2.Bottom() - 7, ANM_CONTROLS_SPR_LOCKS, 2, dwWinColor);
				CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.CenterX() + 10, recttemp2.Bottom() - 4, ANM_CONTROLS_SPR_ICONS_MISC, 1, dwWinColor);
				//paint cost
				CStringDesc sdPrice;
				g_stringsMgr.SetStringDesc(&sdPrice, L"%d", nWeaponPrice);
			...
			CtrlMgrDrawFrame(&UTGetControlsManager().m_sprCol, ANM_CONTROLS_SPR_FRAME_WHITE1, recttemp, D3DCOLOR_COLORALPHA(0xff1C252E, fAlpha));
		...
		case K_PSS_CURPOS_ULTIMATE:
		{
			...
			for (int kk = 0; kk < nItemsCnt; kk++)
			{
				...
				else //locked
				{
					...
					//name
					recttemp2.Set(recttemp.Right() - 72, recttemp.y - 1, 74, 18);
					g_font6n1->DrawString(pItemSm->strIdxScreenName, recttemp2, FONTFLAG_ANCHOR_TOPRIGHT | FONTFLAG_WRAPTEXT, D3DCOLOR_COLORALPHA(K_COLOR_DEFAULT_TEXT_HALFALPHA, 0.4f));
				}
			}
			...
			else
			{
				...
				CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.CenterX() - 14, recttemp2.Bottom() - 7, ANM_CONTROLS_SPR_LOCKS, 1, dwWinColor);
				CSprite::paintFrame(&UTGetControlsManager().m_sprCol, recttemp2.CenterX() + 10, recttemp2.Bottom() - 4, ANM_CONTROLS_SPR_ICONS_MISC, 1, dwWinColor);
				//paint cost
				CStringDesc sdPrice;
				g_stringsMgr.SetStringDesc(&sdPrice, L"%d", nWeaponPrice);
...
void CPlayerSelScr::ApplyUpgradesOnActor(CPlayerCharSelection* pSel, CActor* pDestActor)
{
	if ((pSel == NULL) || (pDestActor == NULL))
		return;

	//write upgrades in log:
	WCHAR logtxt[1024];
	WCHAR cattxt[1024];
	StringCchPrintf(logtxt, 1024, L"ApplyUpgradesOnActor:: Upgrading [%s] bar points [ ", EPSSPlayerClassNames[(int)pSel->eType].text);
	for (int kk = 0; kk < K_PSS_UPGRADE_BARS_CNT; kk++)
	{
		StringCchPrintf(cattxt, 1024, L"%d ", pSel->arrUpgradeBarsPts[kk]);
		StringCchCat(logtxt, 1024, cattxt);
	}
	StringCchCat(logtxt, 1024, L"]");
	LOG(logtxt);
	...
	for (int kk = 0; kk < K_PSS_UPGRADE_BARS_CNT; kk++)
	{
		...
		int nTotalDots = m_arrUpgradeBars[nBarIdx]->nTotalPoints;
		int nFilledDots = pSel->arrUpgradeBarsPts[kk];
		///--- activate generic bar property (percent) ---
		float fFilledPercent = (float)nFilledDots / (float)nTotalDots;
		UINT32 shBarUID = m_arrUpgradeBars[nBarIdx]->shUID.getHash();
		if (shBarUID == GET_FAST_HASH("TEAM_PHYSICAL"))
		{
			pDestActor->fLife += (float)(nFilledDots * 3);
			pDestActor->templateActor.fLife += (float)(nFilledDots * 3);
		}
		else if (shBarUID == GET_FAST_HASH("TEAM_LOGISTICS"))
		{
			//implemented in GiveStrategicPoints
		}
		else if (shBarUID == GET_FAST_HASH("A1_ACCURACY"))
		{
			//implemented in SetActorWeaponPerks after changing the weapon
		}
		else if (shBarUID == GET_FAST_HASH("A2_RECOIL"))
		{
			float fRecoil = g_playerSelScr.GetUpgradeBarPercent(&g_playerSelScr.m_arrPlayers[pDestActor->nPlayerOrdinal], L"A2_RECOIL");
			pDestActor->templateActor.fRecoilModifier = 1.0f - fRecoil * 0.3f;
		}
		else if (shBarUID == GET_FAST_HASH("B1_RECOIL"))  //breacher recoil bar = rate of fire modifier for pump shotguns
		{
			float fPerc = g_playerSelScr.GetUpgradeBarPercent(&g_playerSelScr.m_arrPlayers[pDestActor->nPlayerOrdinal], L"B1_RECOIL");
			for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
			{
				CWeapon* pWeapon = &pDestActor->weapons[jj];
				if ((pWeapon->WeaponTemplate.name.IsEqual(L"WPN_SHOTGUN_M590")) ||
					(pWeapon->WeaponTemplate.name.IsEqual(L"WPN_SHOTGUN_M870")) ||
					(pWeapon->WeaponTemplate.name.IsEqual(L"WPN_SHOTGUN_KSG")))
				{
					pWeapon->WeaponTemplate.fFireRateWait *= 1.0f - fPerc * 0.2;
				}
			}

		}
		else if (shBarUID == GET_FAST_HASH("B2_RELOAD"))  //breacher reload speed
		{
			float fReloadPerc = g_playerSelScr.GetUpgradeBarPercent(&g_playerSelScr.m_arrPlayers[pDestActor->nPlayerOrdinal], L"B2_RELOAD");
			pDestActor->templateActor.fDexterity += fReloadPerc * 0.2f;
		}
		else if (shBarUID == GET_FAST_HASH("F2_MELEE"))  //FBI Agent melee damage
		{
			float fMeleePerc = g_playerSelScr.GetUpgradeBarPercent(&g_playerSelScr.m_arrPlayers[pDestActor->nPlayerOrdinal], L"F2_MELEE");
			pDestActor->weapons[K_LVL_ACT_WEAPON_MELEE].WeaponTemplate.bulletTemplate.fDamage *= 1.0f + fMeleePerc * 0.3f;
		}
		else if (shBarUID == GET_FAST_HASH("S2_SHIELD"))  //shield armor
		{
			float fShieldPerc = g_playerSelScr.GetUpgradeBarPercent(&g_playerSelScr.m_arrPlayers[pDestActor->nPlayerOrdinal], L"S2_SHIELD");
			pDestActor->templateActor.fArmor *= 1.0f + 0.36f * fShieldPerc;
			pDestActor->fArmor = pDestActor->templateActor.fArmor;
		}

		///--- activate bar perks now ---
		for (int ll = 0; ll < nFilledDots; ll++)
		{
			if (m_arrUpgradeBars[nBarIdx]->m_arrPerks[ll].nPointPrice >= 0)
			{
				UINT32 shPerkUID = m_arrUpgradeBars[nBarIdx]->m_arrPerks[ll].shUID.getHash();
#if defined(_DEBUG) || defined(DEBUG) || defined(ENABLE_DEVMODE_RELEASE)
				LOG(L"Adding XP Perk: %s", m_arrUpgradeBars[nBarIdx]->m_arrPerks[ll].shUID.text);
#endif
				//#PERK: SPEED INCREASE Speed is life
				if (shPerkUID == shPerk_SPEED_INCREASE.textHash)
				{
					pDestActor->templateActor.moveMinSpeed *= 1.2f;
					pDestActor->templateActor.moveBackSpeed *= 1.2f;
					pDestActor->templateActor.moveMaxSpeed *= 1.2f;
				}
				//#PERK: POINTMAN ARMOR 50% more armor
				else if (shPerkUID == shPerk_POINTMAN_ARMOR.textHash)
				{
					pDestActor->templateActor.fArmor *= 1.5f;
					pDestActor->fArmor = pDestActor->templateActor.fArmor;
				}
				//#PERK: MULE - extra gear
				else if (shPerkUID == shPerk_MULE.textHash)
				{
					CWeapon* pGearWpn = pDestActor->pSelectedWeapon[K_LVL_ACT_WEAPON_GEAR];
					...
					{
						pGearWpn->WeaponTemplate.nClipSize += 1;
						pGearWpn->ammoLeft += 1;
					}
				}
				//#PERK: CRITICAL SHOTS Assaulter
				else if (shPerkUID == shPerk_CRITICAL_SHOTS.textHash)
				{
					//only weapons that have critical chances get the perk
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.fCriticalHitChance > 0.0f)
							pWeapon->WeaponTemplate.bulletTemplate.fCriticalHitChance += 0.05f;
					}
				}
				//#PERK: C-CLAMP Assaulter
				else if (shPerkUID == shPerk_C_CLAMP.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						pWeapon->WeaponTemplate.fAimErrorCooldownPerSecond *= 1.35f;
					}
				}
				//#PERK: SUREFOOTED
				else if (shPerkUID == shPerk_SUREFOOTED.textHash)
				{
					CWeapon* pAltWpn = pDestActor->pSelectedWeapon[K_LVL_ACT_WEAPON_SECONDARY];
					if (pAltWpn->WeaponTemplate.name.IsEqual(L"WPN_SUBMACHINE_STEADY"))
					{
						pAltWpn->WeaponTemplate.fShooterSpeedSlowingPercent = 0.6f;
					}
				}
				//#PERK: 9 PELLET BUCK - shotguns have more pellets
				else if (shPerkUID == shPerk_9PELLET_BUCK.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.nBulletsPerShot > 1)
						{
							pWeapon->WeaponTemplate.nBulletsPerShot += 1;
						}
					}
				}
				//#PERK: EXTENSION TUBES - one extra bullet for tube shotguns
				else if (shPerkUID == shPerk_EXTENSION_TUBES.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.nReloadUnitSize == 1)
						{
							pWeapon->WeaponTemplate.nClipSize += 1;
							pWeapon->ammoLeft += 1;
						}
					}
				}
				//#PERK: RIFLED SLUGS
				else if (shPerkUID == shPerk_RIFLED_SLUGS.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nType == K_LVL_BULLET_SHOTGUN_SLUG)
						{
							pWeapon->WeaponTemplate.fSpreadFOV *= 0.5f;
						}
					}
				}
				//#PERK: IMPROVED SLUGS
				else if (shPerkUID == shPerk_IMPROVED_SLUGS.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nType == K_LVL_BULLET_SHOTGUN_SLUG)
						{
							pWeapon->WeaponTemplate.bulletTemplate.fDamage *= 1.5f;
						}
					}
				}
				//#PERK: EXTENDED MAGS - +2 ammo on pistols (not revolvers without bullet chambers)
				else if (shPerkUID == shPerk_EXTENDED_MAGS.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if ( (pWeapon->WeaponTemplate.nReloadUnitSize > 1) &&
							 (pWeapon->WeaponTemplate.nBulletChamberSize > 0) )
						{
							pWeapon->WeaponTemplate.nClipSize += 2;
							pWeapon->WeaponTemplate.nReloadUnitSize += 2;
							pWeapon->ammoLeft += 2;
						}
					}
				}
				//#PERK: PLATES UPGRADE
				else if (shPerkUID == shPerk_PLATES_UPGRADE.textHash)
				{
					if(pDestActor->fArmor > 0.0f)
						pDestActor->templateActor.nArmorRating += 1;
				}
				//#PERK: GRIZZLY - movement speed
				else if (shPerkUID == shPerk_GRIZZLY.textHash)
				{
					pDestActor->templateActor.moveMinSpeed += pDestActor->templateActor_ini.moveMinSpeed * 0.1f;
					pDestActor->templateActor.moveBackSpeed += pDestActor->templateActor_ini.moveBackSpeed * 0.1f;
					pDestActor->templateActor.moveMaxSpeed += pDestActor->templateActor_ini.moveMaxSpeed * 0.1f;
				}
				//#PERK: TWIXIE - gives extra twinkie to offduty guy
				else if (shPerkUID == shPerk_TWIXIE.textHash)
				{
					CWeapon* pWpn = pDestActor->pSelectedWeapon[K_LVL_ACT_WEAPON_GEAR];
					if (pWpn->WeaponTemplate.name.IsEqual(L"WPN_TWINKIE"))
					{
						pWpn->WeaponTemplate.nClipSize = 2;
						pWpn->ammoLeft = 2;
					}
				}
				//#PERK: FOUGHT OFF A REDHEAD - increase melee push and damage
				else if (shPerkUID == shPerk_REDHEAD.textHash)
				{
					pDestActor->weapons[K_LVL_ACT_WEAPON_MELEE].WeaponTemplate.bulletTemplate.fStunDuration *= 1.5f;
					pDestActor->weapons[K_LVL_ACT_WEAPON_MELEE].WeaponTemplate.bulletTemplate.fMomentum *= 1.5f;
				}
				//#PERK: LOCKED OUTSIDE WINTER - increase melee object damage
				else if (shPerkUID == shPerk_LOCKED_OUT.textHash)
				{
					pDestActor->weapons[K_LVL_ACT_WEAPON_BREACH].WeaponTemplate.bulletTemplate.fDamageObjects *= 1.4f;
					pDestActor->weapons[K_LVL_ACT_WEAPON_MELEE].WeaponTemplate.bulletTemplate.fDamageObjects *= 1.4f;
				}
				//#PERK: SOFT POINTS (offduty guy) - increase damage for all rifles
				else if (shPerkUID == shPerk_SOFT_POINTS.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						...
						{
							pWeapon->WeaponTemplate.bulletTemplate.fDamage *= 1.2f;
						}
					}
				}
				//#PERK: P AMMO (offduty guy) - increase damage for all rifles and remove "die on impact" flag
				else if (shPerkUID == shPerk_P_AMMO.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if ((pWeapon->WeaponTemplate.bulletTemplate.nGroup == K_LVL_BULLGROUP_BULLETS) &&
							(pWeapon->WeaponTemplate.nBulletsPerShot == 1) && 
							(pWeapon->WeaponTemplate.bHasLaserSight == false))
						{
							pWeapon->WeaponTemplate.bulletTemplate.fSelfDamageMultiplier = 0.85f;
							pWeapon->WeaponTemplate.bulletTemplate.nFlags &= ~K_LVL_BULLET_FLAG_DIE_ON_IMPACT;
						}
					}
				}
				//#PERK: OPEN SEASON (offduty guy) - increase damage for all rifles
				else if (shPerkUID == shPerk_OPEN_SEASON.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nGroup == K_LVL_BULLGROUP_BULLETS)
						{
							pWeapon->WeaponTemplate.bulletTemplate.fCriticalHitChance += 0.3f;
						}
					}
				}
				//#PERK: EXTENDED MAGS RECON - +5 ammo on all guns
				else if (shPerkUID == shPerk_EXTENDED_MAGS_RECON.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.nReloadUnitSize > 1)
						{
							pWeapon->WeaponTemplate.nClipSize += 5;
							pWeapon->WeaponTemplate.nReloadUnitSize += 5;
							pWeapon->ammoLeft += 5;
						}
					}
				}
				//#PERK: HOLLOW POINTS - FBI +25% damage
				else if (shPerkUID == shPerk_HOLLOW_POINTS.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nGroup == K_LVL_BULLGROUP_BULLETS)
						{
							pWeapon->WeaponTemplate.bulletTemplate.fDamage *= 1.25f;
						}
					}
				}
				//#PERK: WEAVER STANCE - improved recoil
				else if (shPerkUID == shPerk_WEAVER_STANCE.textHash)
				{
					pDestActor->templateActor.fRecoilModifier -= 0.35f;
					CLAMP(pDestActor->templateActor.fRecoilModifier, 0.0f, 1.0f);
				}
				//#PERK: LEG DAY - FBI breach
				else if (shPerkUID == shPerk_LEG_DAY.textHash)
				{
					CWeapon* pWeapon = &pDestActor->weapons[K_LVL_ACT_WEAPON_BREACH];
					pWeapon->WeaponTemplate.bulletTemplate.fDamageObjects = 35.0f;
				}
				//#PERK: SHIELD BRACE - improved recoil
				else if (shPerkUID == shPerk_SHIELD_BRACE.textHash)
				{
					pDestActor->templateActor.fRecoilModifier -= 0.4f;
					CLAMP(pDestActor->templateActor.fRecoilModifier, 0.0f, 1.0f);
				}
				//#PERK: ARMORED RABBIT
				else if (shPerkUID == shPerk_ARMORED_RABBIT.textHash)
				{
					pDestActor->templateActor.moveMinSpeed *= 1.155f;
					pDestActor->templateActor.moveBackSpeed *= 1.155f;
					pDestActor->templateActor.moveMaxSpeed *= 1.155f;
				}
				//#PERK: SURPRISE CRIT - critical hit on surprised enemies
				else if (shPerkUID == shPerk_LADY_JUSTICE.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nGroup == K_LVL_BULLGROUP_BULLETS)
						{
							pWeapon->WeaponTemplate.bulletTemplate.nFlags |= K_LVL_BULLET_FLAG_CRITICAL_IF_SCARED;
						}
					}
				}
				//#PERK: CADDYSHACK - double gear
				else if (shPerkUID == shPerk_CADDYSHACK.textHash)
				{
					CWeapon* pGearWpn = pDestActor->pSelectedWeapon[K_LVL_ACT_WEAPON_GEAR];
					if ((pGearWpn != null) && (pGearWpn->status != K_LVL_WPN_STATUS_UNKNOWN) && (pGearWpn->WeaponTemplate.nClipSize > 0))
					{
						pGearWpn->WeaponTemplate.nClipSize *= 2;
						pGearWpn->ammoLeft = pGearWpn->WeaponTemplate.nClipSize;
					}
				}
				//#PERK: DOUBLE TOOLS - double recon tools (spy cam and cam balls)
				else if (shPerkUID == shPerk_DOUBLE_TOOLS.textHash)
				{
					CWeapon* pGearWpn = pDestActor->pSelectedWeapon[K_LVL_ACT_WEAPON_GEAR];
					if ((pGearWpn != null) && (pGearWpn->status != K_LVL_WPN_STATUS_UNKNOWN) && (pGearWpn->WeaponTemplate.nClipSize > 0))
					{
						if ((pGearWpn->WeaponTemplate.name.IsEqual(L"WPN_SPY_CAMERA")) || (pGearWpn->WeaponTemplate.name.IsEqual(L"WPN_CAM_BALL")))
						{
							pGearWpn->WeaponTemplate.nClipSize *= 2;
							pGearWpn->ammoLeft = pGearWpn->WeaponTemplate.nClipSize;
						}
				...
				//#PERK: CRIPPLING SHOTS
				else if (shPerkUID == shPerk_CRIPPLING_SHOTS.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nGroup == K_LVL_BULLGROUP_BULLETS)
						{
							pWeapon->WeaponTemplate.bulletTemplate.nFlags |= K_LVL_BULLET_FLAG_CAN_CRIPPLE;
						}
					}
				}
				//#PERK: SURGEON PRECISION
				else if (shPerkUID == shPerk_SURGEON_PRECISION.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nGroup == K_LVL_BULLGROUP_BULLETS)
						{
							pWeapon->WeaponTemplate.bulletTemplate.nFlags |= K_LVL_BULLET_FLAG_SURGEON_IF_TARGETED;
						}
					}
				}
				//#PERK: VULTURE - crit prob when shot from behind
				else if (shPerkUID == shPerk_VULTURE.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if (pWeapon->WeaponTemplate.bulletTemplate.nGroup == K_LVL_BULLGROUP_BULLETS)
						{
							pWeapon->WeaponTemplate.bulletTemplate.nFlags |= K_LVL_BULLET_FLAG_CRITICAL_FROM_BEHIND;
						}
					}
				}
				//#PERK: HANDYMAN - 0.5 seconds faster reload
				else if (shPerkUID == shPerk_HANDYMAN.textHash)
				{
					for (int jj = 0; jj < K_LVL_ACT_WEAPONS_CNT; jj++)
					{
						CWeapon* pWeapon = &pDestActor->weapons[jj];
						if ((pWeapon->WeaponTemplate.nReloadUnitSize > 0) && (pWeapon->WeaponTemplate.name.IsSet()))
						{
							pWeapon->WeaponTemplate.fReloadTimePerUnit -= 0.5f;
							if (pWeapon->WeaponTemplate.fReloadTimePerUnit < 0.5f)
								pWeapon->WeaponTemplate.fReloadTimePerUnit = 0.5f;
						}
					}
				}
				//#PERK: DISORIENTED - more knockback for alt fire (RECON usually)
				else if (shPerkUID == shPerk_DISORIENTED.textHash)
				{
					CWeapon* pAltWpn = pDestActor->pSelectedWeapon[K_LVL_ACT_WEAPON_SECONDARY];
					if (pAltWpn->WeaponTemplate.bulletTemplate.nType == K_LVL_BULLET_TRACER_RECON)
					{
						pAltWpn->WeaponTemplate.bulletTemplate.fStunDuration *= 1.2f;
						pAltWpn->WeaponTemplate.bulletTemplate.fMomentum *= 2.0f;
					}
				}

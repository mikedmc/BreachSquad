...
void CControllerData::MapKeys(UINT nLeftKey, UINT nRightKey, UINT nUpKey, UINT nDownKey, UINT nJumpKey, UINT nFire1Key, UINT nFireAltKey, UINT nReloadKey, UINT nUseGearKey)
{
	keyMappings[K_KEY_LEFT] = nLeftKey;
	keyMappings[K_KEY_RIGHT] = nRightKey;
	keyMappings[K_KEY_UP] = nUpKey;
	keyMappings[K_KEY_DOWN] = nDownKey;
	keyMappings[K_KEY_JUMP] = nJumpKey;
	keyMappings[K_KEY_FIRE1] = nFire1Key;
	keyMappings[K_KEY_FIRE2] = nFireAltKey;
	keyMappings[K_KEY_RELOAD] = nReloadKey;
	keyMappings[K_KEY_USE_GEAR] = nUseGearKey;
}

void CControllerData::MapKey(EControllerKeys nKeyType, UINT nKey)
{
	keyMappings[nKeyType] = nKey;
}

//--------------------------------------------------------------------------------------
// Controller handling class
//--------------------------------------------------------------------------------------
void CControllerData::ResetKeypresses()
{
...
void CControllerData::ReceiveKeypress(UINT nChar, bool bIsKeyDown, bool bAltDown)
{
	for (int kk = 0; kk < K_KEYS_COUNT; kk++)
	{
		if (keyMappings[kk] == nChar)
		{
			bKeyDown[kk] = bIsKeyDown;
			return;
		}
	}
}

void CControllerData::Update(float dTime)
{
	for (int kk = 0; kk < K_KEYS_COUNT; kk++)
	{
		//update pressed time
		...
		if (bKeyDown[kk])
		{
			if (keyState[kk] == K_KEYSTATE_NOTPRESSED)
			{
				keyState[kk] = K_KEYSTATE_JUSTPRESSED; //just pressed
			}
			else if (keyState[kk] == K_KEYSTATE_JUSTPRESSED)
			{
				keyState[kk] = K_KEYSTATE_PRESSING; //drag
			}
		}
		else
		{
			if (keyState[kk] == K_KEYSTATE_PRESSING)
			{
				keyState[kk] = K_KEYSTATE_JUSTRELEASED;
			}
			else
			{
				keyState[kk] = K_KEYSTATE_NOTPRESSED;

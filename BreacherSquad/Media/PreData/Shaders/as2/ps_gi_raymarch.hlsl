...
void fragment() 
{
	...
	for(float i = 0.0; i < u_rays_per_pixel; i++)
	{
		...
		//MIKE: aici in loc de cos sin ar trebui luat din blue noise vector si facuta normalizare optimizat: float invLen = inversesqrt(dot(dir, dir)); vec2 approxDir = dir * invLen;

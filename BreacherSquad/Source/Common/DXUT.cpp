...
public:
	...
	void Create()
	{
		...
		m_state.m_AllowShortcutKeysWhenWindowed = false;
...
HRESULT DXUTInit(bool bParseCommandLine, bool bHandleDefaultHotkeys, bool bShowMsgBoxOnError)
{
	...
	GetDXUTState().SetStartupToggleKeys(tk);  //caps lock, num lock
...
void DXUTParseCommandLine()
{
	...
	for(;;)
	{
...
#ifdef ENABLE_STEAM
		//#DMC: Steam lobby param command line reading
 		StringCchCopy(strFlag, 256, L"+connect_lobby"); nFlagLen = (int)wcslen(strFlag);
		if (_wcsnicmp(strCmdLine, strFlag, nFlagLen) == 0)
		{
			//add ':' after +connect_lobby so we can read it using DXUTGetCmdParam
			strCmdLine[nFlagLen] = ':';
			//read steam id
			if (DXUTGetCmdParam(strCmdLine, strFlag, nFlagLen))
			{
				g_netlock.m_ullCurLobbyID = _wtoi64(strFlag);
				//set startup command
				g_startupCommand = GAME_STARTUP_JOIN_LOBBY;
				g_startupParam.Init(strFlag);
			}
			continue;
		}
#endif
...
#ifdef ENABLE_GALAXY
		//#DMC: Galaxy lobby param command line reading format: -connect-lobby-17346409328 so we replace third "-" with ":"
		StringCchCopy(strFlag, 256, L"connect-lobby-"); nFlagLen = (int)wcslen(strFlag);
		if (_wcsnicmp(strCmdLine, strFlag, nFlagLen) == 0)
		{
			strCmdLine[nFlagLen - 1] = ':';
			//remove last '-' from strFlag so we can parse it as ususal
			strFlag[nFlagLen - 1] = 0;
			nFlagLen--;
			...
			if (!g_netlock.m_ullCurLobbyID.IsValid())
			{
				LOG(L"Error: Trying to parse Galaxy -connect-lobby-<lobby_id> cmd - cannot read lobby id!\n");
			}
			else
			{
				//set startup command
		...
		// check for lobby param passed through command line
		for (int i = 0; i < numCmds; ++i)
		{
		if (strstr(cmdLine[i], "-connect-lobby-") == cmdLine[i])
		{
		sscanf(cmdLine[i], "-connect-lobby-%llu", &Options::ullJoinLobbyID);
		if (!Options::ullJoinLobbyID.IsValid())
		g_pLog->Write("Error: Trying to parse Galaxy -connect-lobby-<lobby_id> cmd - cannot read lobby id!\n");
		break;
		}
		}
		*/
		...
		//#DMC: Loads specified level immediately
		StringCchCopy(strFlag, 256, L"+map"); 
		nFlagLen = (int)wcslen(strFlag);
		if (_wcsnicmp(strCmdLine, strFlag, nFlagLen) == 0)
		{
			//copy all string as path
			WCHAR *wcsPathPtr = strCmdLine + nFlagLen + 1; //skip the first space too
			WCHAR wcsPath[MAX_PATH];
			StringCchCopy(wcsPath, MAX_PATH, wcsPathPtr);
			//set startup command
			g_startupCommand = GAME_STARTUP_LOAD_MAP;
			g_startupParam.Init(wcsPath);
			//advance pointer till end
			strCmdLine += wcslen(strCmdLine);
			continue;
		}

#if defined(ENABLE_STEAM_WORKSHOP)
		//#DMC: Uploads specified file to workshop
		StringCchCopy(strFlag, 256, L"+upload_workshop"); nFlagLen = (int)wcslen(strFlag);
		if (_wcsnicmp(strCmdLine, strFlag, nFlagLen) == 0)
		{
			//copy all string as path
			WCHAR *wcsPathPtr = strCmdLine + nFlagLen + 1; //skip the first space too
			...
			StringCchCopy(wcsPath, MAX_PATH, wcsPathPtr);
			//set startup command
			g_startupCommand = GAME_STARTUP_UPLOAD_MOD;
			g_startupParam.Init(wcsPath);
			//advance pointer till end
			strCmdLine += wcslen(strCmdLine);
			continue;
		...
		*strSpace = 0;
		...
		LOG(L"Unrecognized flag: %s", strFlag);
						 ...
						 HICON hIcon, HMENU hMenu, int x, int y)
{
	...
	if (DXUTGetHWNDFocus() == NULL)
	{
		...
		wndClass.lpszClassName = K_GAME_WINDOW_CLASSNAME;
		...
		// be resized later
		//#DMC : nu se face resize la fereastra ---
		...
		int nDefaultWidth = K_WINDOW_WIDTH_MIN;
		int nDefaultHeight = K_WINDOW_HEIGHT_MIN;
...
//		DebugLogA("CreateWin::AdjustWinRect w=%d, h=%d\n", rc.right, rc.bottom);

		WCHAR* strCachedWindowTitle = GetDXUTState().GetWindowTitle();
		StringCchCopy(strCachedWindowTitle, 256, strWindowTitle);

		// Create the render window
		HWND hWnd = CreateWindow(K_GAME_WINDOW_CLASSNAME, strWindowTitle, dwWindowStyle,
						 ...
						 LPDXUTCALLBACKMODIFYDEVICESETTINGS pCallbackModifyDeviceSettings)
{
	...
	matchOptions.eResolution         = /*DXUTMT_PRESERVE_INPUT;*/DXUTMT_CLOSEST_TO_INPUT;
	...
	matchOptions.ePresentInterval    = /*DXUTMT_IGNORE_INPUT;*/ DXUTMT_CLOSEST_TO_INPUT;
...
HRESULT DXUTToggleFullScreen()
{
	...
	//#DMC #HACK: force fullscreen on main monitor
	if (!deviceSettings.pp.Windowed)
	{
		deviceSettings.AdapterOrdinal = 0;
	}
	...
	matchOptions.eDeviceType         = DXUTMT_CLOSEST_TO_INPUT;
	matchOptions.eWindowed           = DXUTMT_PRESERVE_INPUT;
	matchOptions.eAdapterFormat      = DXUTMT_PRESERVE_INPUT;//DXUTMT_IGNORE_INPUT;
	matchOptions.eVertexProcessing   = DXUTMT_CLOSEST_TO_INPUT;
	matchOptions.eBackBufferFormat   = DXUTMT_IGNORE_INPUT;
	matchOptions.eBackBufferCount    = DXUTMT_CLOSEST_TO_INPUT;
	matchOptions.eMultiSample        = DXUTMT_CLOSEST_TO_INPUT;
	...
	if (deviceSettings.pp.Windowed)
	{
		...
		DebugPrintA("ToggleFS():Windowed nW:%d nH:%d\n", nWidth, nHeight);
	}
	else //Fullscreen
	{
	...
	if (SUCCEEDED(hr)) 
	{
//		DebugLogA("success FindValidDevSett\n");
...
//			DebugLogA("ChangeDev failed\n");
			...
			{
				matchOptions.eResolution = /*DXUTMT_PRESERVE_INPUT;*/DXUTMT_IGNORE_INPUT;
			}
			...
			if (FAILED(hr2))
			{
				ErrorBox(K_ERR_CRITICAL, L"DXUTToggleFullscreen::Couldn't change to desired resolution! Stopped trying!");
									...
									DXUTMatchOptions* pMatchOptions)
{
	...
	// Back buffer count
	//---------------------
	//DMC: pOptimalDeviceSettings->pp.BackBufferCount = 1;
	...
	// Multisample
	//---------------------
	//TODO: de setat aceste chestii din modifyDeviceSettings
	// pus default pe no multisampling
...
*/
	...
	// Present interval
	//---------------------
	UINT bestPresentInterval = D3DPRESENT_INTERVAL_DEFAULT;
...
HRESULT DXUTChangeDevice(DXUTDeviceSettings* pNewDeviceSettings, IDirect3DDevice9* pd3dDeviceFromApp, bool bForceRecreate)
{
	...
	{
//		DebugLogA("bbuffer w:%d, h%d\n", pNewDeviceSettings->pp.BackBufferWidth, pNewDeviceSettings->pp.BackBufferHeight);
		...
		if (pNewDeviceSettings->pp.BackBufferWidth < K_WINDOW_WIDTH_MIN)
			pNewDeviceSettings->pp.BackBufferWidth = K_WINDOW_WIDTH_MIN;
		if (pNewDeviceSettings->pp.BackBufferHeight < K_WINDOW_HEIGHT_MIN)
			pNewDeviceSettings->pp.BackBufferHeight = K_WINDOW_HEIGHT_MIN;

//		DebugLogA("altered bbuffer w:%d, h%d\n", pNewDeviceSettings->pp.BackBufferWidth, pNewDeviceSettings->pp.BackBufferHeight);
...
//		DebugLogA("ChangeDevice::AdjustWinRect w=%d, h=%d\n", rcWindowClient.right, rcWindowClient.bottom);
	...
	if (pNewDeviceSettings->pp.Windowed)
	{
		...
		//DMC: salveaza dimensiunile deskului
		//g_mainMonitorWidth = nMonitorWidth;
		//g_mainMonitorHeight = nMonitorHeight;
...
void DXUTAllowShortcutKeys(bool bAllowKeys)
{
	...
	if (bAllowKeys)
	{
		...
		//#DMC: In cazul in care dai alt+tab sa nu mai incetineasca celelalte programe
...
void DXUTPause(bool bPauseTime, bool bPauseRendering)
{
	...
	LOG(L"DXUT::Paused(Time:%d Render:%d)", nPauseTimeCount, nPauseRenderingCount);
}
...
//--- aici verifica capabilitatile placii : adaugat de mine ---
	//TODO: trebuie scos de aici si mutat in verifySettings in UTAppClass
	D3DCAPS9* pd3dCaps = new D3DCAPS9();
	ZeroMemory(pd3dCaps, sizeof(pd3dCaps));

	WCHAR wszText[2048] = {0};
	LPDIRECT3DDEVICE9 pd3dDevice = GetDXUTState().GetD3DDevice();
	if(!FAILED(pd3dDevice->GetDeviceCaps(pd3dCaps)))
	{
		if((pd3dCaps->MaxTextureWidth < 1024) || (pd3dCaps->MaxTextureHeight < 1024))
		{
			hr = E_FAIL;
			WCHAR wsz[256];
			StringCchPrintf( wsz, 256, L"- Your graphics card doesn't support large textures (1024x1024) or it doesn't have enough video memory\n");
			StringCchCat( wszText, 2048, wsz );
			//sterge flag texturi mari
			//g_cardFlags &= ~CARD_FLAG_1024TEXTURES;
		}
		//else
			//g_cardFlags |= CARD_FLAG_1024TEXTURES;

		if((pd3dCaps->AlphaCmpCaps & D3DPCMPCAPS_GREATEREQUAL) == 0)
		{
			//g_cardFlags &= ~CARD_FLAG_SUPPORTALPHATEST;
		}
		//else
			//g_cardFlags |= CARD_FLAG_SUPPORTALPHATEST;
		...
		if(FAILED(hr))
		{
			StringCchCat( wszText, 2048, L"\n The application may not work properly if run.  Do you wish to continue?" );
...
void DXUTRender3DEnvironment()
{
	...
	if (GetDXUTState().GetDeviceLost() && !DXUTIsRenderingPaused()/*GetDXUTState().GetRenderingPaused()*/)
	{
	...
	//DebugLogFnA(" try to present scene \n");
	...
	*/
	//DebugLogFnA(" present scene ENDED \n");
...
LRESULT CALLBACK DXUTStaticWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		else if (SIZE_RESTORED == wParam)
		{
			...
			else
			{
				...
				//#IMPORTANT #DMC: va verifica si aici size-change pentru ca nu permitem scalare prin corner drag ci doar din meniu
				//in cazul in care las scalabila fereastra trebuie scoasa linia asta de aici
				DXUTHandlePossibleSizeChange();
			}
		}
		break;

		//ca sa nu poti sa scalezi fereastra
	case WM_GETMINMAXINFO:
		((MINMAXINFO*)lParam)->ptMinTrackSize.x = K_WINDOW_WIDTH_MIN;
		((MINMAXINFO*)lParam)->ptMinTrackSize.y = K_WINDOW_HEIGHT_MIN;
		//trebuie setat neaparat
		((MINMAXINFO*)lParam)->ptMaxTrackSize.x = 6400;
		((MINMAXINFO*)lParam)->ptMaxTrackSize.y = 3600;
		break;
	...
	case WM_SETCURSOR:
		...
		if ( (GetDXUTState().GetActive()) && (!DXUTIsRenderingPaused()) )
		{
			if(!DXUTIsWindowed())
			{
				//SetCursor(NULL);  <-- nu trebuie sa fie aici
				IDirect3DDevice9* pd3dDevice = DXUTGetD3DDevice();
			...
			//else //adaugat DMC
			//{
			//	if((g_bCustomMouseCursor) && (!g_bCursorOutsideWindow))
			//	{
	...
	case WM_ACTIVATEAPP:
		if (wParam == TRUE)
		{
			...
			//DMC: adaugat aici sa iasa din pauza
			DXUTPause(false, false);
		}
		else 
		{
			...
			//DMC: am adaugat pauza pe lost focus, daca nu este deja pauza pusa
...
#define PBT_APMRESUMESUSPEND 0x0007
				...
				case VK_RETURN:
					{
						...
						if ((lParam & dwMask) != 0)
						{
							...
							//cateodata cand revine in windowed mode fereastra e prea in afara ecranului. Asa o repozitioneaza in ecran
							if (DXUTIsWindowed())
							{
	...
	case WM_CLOSE:
		{
			...
			if (hMenu != NULL)
				DestroyMenu(hMenu);
			DestroyWindow(hWnd);
			UnregisterClass(K_GAME_WINDOW_CLASSNAME, NULL);
	...
	//sa nu acceseze meniul cand apesi pe ALT
	if (GetDXUTState().GetMenu() == NULL && (uMsg == WM_SYSKEYDOWN || uMsg == WM_SYSKEYUP) && (lParam & (1<<29)))
	{
		return 0;
	}

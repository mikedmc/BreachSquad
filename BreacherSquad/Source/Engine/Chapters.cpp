...
bool CChaptersList::LoadChapters(WCHAR * strXmlPath)
{
	...
	for (pugi::xml_node chapternode = rootnodeexplo.first_child(); chapternode; chapternode = chapternode.next_sibling())
	{
		...
		cdesc->nLevelsCnt = 0;
		//get chapter data
		cdesc->nChapterNameStrIdx = __Texts().GetStrIdx(FastHash(chapternode.attribute(L"sStrID_ChapterName").value()));
		...
		{
			cdesc->arrLevelNameStrIdx[cdesc->nLevelsCnt] = __Texts().GetStrIdx(missionnode.attribute(L"sStrID_LevelName").value());

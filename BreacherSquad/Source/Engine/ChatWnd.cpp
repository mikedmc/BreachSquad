...
void CChatWnd::AddLine(WCHAR * strText, WCHAR * strAuthor, DWORD dwColor /*= 0xffffffff*/)
{
	...
	{
		if ((UTApp().IsGameNetworked()) && (__Sim().m_levelState == K_LVL_STATE_PLAYING))
		{
			LOG(L"[NET] CChatWnd::AddLine received command [%s]", strText);
			__Sim().SetLevelState(K_LVL_STATE_MISSION_FAILED, STR_MISSION_ABORTED);
		}
		return;
	}
...
void CChatWnd::Paint(D3DXVECTOR2 vBottomLeft)
{
	...
	if (bReceivingText)
	{
		...
		GUIUtils::DrawWindowFrame(&__GUI().m_sprCol, ANM_CONTROLS_SPR_FRAME1, rctxywh, 0xffffffff);
		//draw text

...
void CLevel::UpdateAI_actor(CActor* actor, float dTime)
{
	...
	*/
	...
	if (actor->eAttackStatus >= K_ACT_ATTACK_SHOOTING)
	{
		...
		{
			dwShootScriptUID = pWeaponMain->_template.shScript_OnFire.getHash();
		}
	...
	//#TODO: salveaza referinta la nivel din __Sim decat sa chemi de fiecare data functia
	actor->Update(dTime, __Sim());
}

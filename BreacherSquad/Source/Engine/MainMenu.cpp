	...
	switch (m_eState)
	{
		...
		case K_MM_STATE_MAINMENU:
		{
			...
			*/
			...
			if (g_timers.Tick(90))
			{
				__Particles().AddParticle(ANM_PARTICLES_SPR_FIRE_PARTS1, false, randint(9), &D3DXVECTOR2(worldrect.Right() + 10.0f, randfloat(worldrect.h / 2.0f) + worldrect.h * 0.25f),
					&D3DXVECTOR2(-4.0f, -8.0f), &D3DXVECTOR2(-40.0f - randfloat(20.0f), -10.0f - randfloat(10.0f)), 3.0f + randfloat(2.0f), 0.6f - randfloat(0.2f), -0.05f, 0.0f, randfloatsgn(PI), 0.1f, 1.0f, 0xffffffff, K_PART_LAYER_NORMAL_LIGHT);
			}

...
void CMenus::Update( float dTime )
{
	...
	if ( g_timers.Tick(90) )
	{
		__Particles().AddParticle(ANM_PARTICLES_SPR_FIRE_PARTS1, false, randint(12), &D3DXVECTOR2(worldrect.Right() + 10.0f, randfloat(worldrect.h / 2.0f) + worldrect.h * 0.25f),
	...
	{
		__Particles().AddParticle(ANM_PARTICLES_SPR_FIRE_PARTS1, false, randint(12), &D3DXVECTOR2(worldrect.Right() + 10.0f, randfloat(worldrect.h / 2.0f) + worldrect.h * 0.25f),
			&D3DXVECTOR2(-8.0f, -4.0f), &D3DXVECTOR2(-40.0f - randfloat(20.0f), -10.0f - randfloat(10.0f)), 3.0f + randfloat(2.0f), 0.5f - randfloat(0.2f), -0.05f, 0.0f, randfloatsgn(PI), 0.1f, 1.0f, 0xaaffffff, K_PART_LAYER_FRONT_LIGHT);
	}
}

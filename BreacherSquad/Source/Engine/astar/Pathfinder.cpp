...
void Pathfinder::Init(int sourceWidthMeters, int sourceHeightMeters, unsigned char blockMask)
{
	...
	LOG( "AStar map allocated. %d x %d", newWidth, newHeight );
...
void Pathfinder::FloodfillAccessibleMask()
{
	...
	while ( toVisit.GetNumElements() ) {
		...
		if ( x != 0 && !( m_nodeData[y*m_width + x - 1] & ( COL_ACCESSIBLE | COL_MOVEMENT_BLOCK ) ) ) {
			_ASSERT( !( m_nodeData[y*m_width + x - 1] & COL_MOVEMENT_BLOCK ) );
...
bool Pathfinder::TraceLine(Vec2 start, Vec2 end, unsigned blockFlags, Vec2& outWS) const
{
	...
	if(hit)
		outWS = ConvertToWorldCoords(out.x, out.y);
	return hit;
}
...
{
	unsigned data = GetRawData_Safe(pos);
...
bool Pathfinder::ClosestBlockedCellOutsideRadius(Vec2& out, Vec2 pos, unsigned blockFlags, const Vec2* occupiedPos, float occupiedRadius, int count) const
{
	...
	do {
		...
		{
			Vec2 p = ConvertToWorldCoords( px, py);
			bool occupied = false;
			for (int i = 0; i < count; i++)
				if (UTMath::Vec2DistSq(occupiedPos[i], p) < occupiedRadius*occupiedRadius) {
					occupied = true;
					break;
				}
			if (!occupied) {
...
bool Pathfinder::ClosestEmptyCell(Vec2& out, Vec2 pos, unsigned blockFlags) const
{
	...
	int bestDir = 0;
	Vec2 dp = pos - ConvertToWorldCoords(pf.x, pf.y);
	...
	do {
		...
		if (spiral.getIdx(pf.x, pf.y, bestDir, px, py)
			&& ((m_nodeData[py * m_width + px] & blockFlags) == 0))
		{
			out = ConvertToWorldCoords(px, py);
			return true;
		}
	} while (spiral);
...
unsigned int Pathfinder::GetRawData_Safe(Vec2 p) const
{
	const Vec2i pf = ConvertToPathfinderCoords(p.x, p.y);
...
bool Pathfinder::GetPath(const Vec2& start, const Vec2& end, Vec2* pPath, int maxPathPoints, int& numPathPoints, unsigned char blockFlags, bool bGetClosestPointIfBlocked /*= true*/, unsigned char additionalCostFlags /*= 0*/, unsigned char additionalSmoothingCollisionFlags )
{
	eResult result = GetPath(ConvertToPathfinderCoords(start.x, start.y), ConvertToPathfinderCoords(end.x, end.y), pPath, maxPathPoints, numPathPoints, blockFlags, bGetClosestPointIfBlocked, additionalCostFlags, additionalSmoothingCollisionFlags);
	if (result == RESULT_ALL_GOOD)
	{
...
Pathfinder::eResult Pathfinder::GetPath(const Vec2i start, Vec2i end, Vec2* pPath, int maxPathPoints, int& numPathPoints, unsigned char blockFlags, bool bGetClosestPointIfBlocked, unsigned char additionalCostFlags, unsigned char additionalSmoothingCollisionFlags )
{
	_ASSERT(blockFlags);
	if (!blockFlags)
...
*/
	...
	if (resultNode == nullptr && bGetClosestPointIfBlocked)
	{
	...
	if (node)
	{
		...
		node = node->parent > -1 ? &m_nodemap[node->parent] : nullptr;
	}
	...
	while (node != nullptr)
	{
		...
		const bool bCollided = TraceBresenhamLine(prevAddedPoint, Vec2i(node->x, node->y), blockFlags | additionalSmoothingCollisionFlags );
		if (bCollided)
		{

...
// this will define how big the collision map will be (set to 0.5 to have 4 cells for one tile)
...
public:
	...
	//PINTEA: when loading a map, call Init(), use AddObject()/MarkShapeCells(etc) for each entity that blocks movement and ComputeClearance() after that.
	void					Init(int sourceWidthMeters, int sourceHeightMeters, unsigned char blockMask);
	void					Release();
	// DMC: Optional/useless for now because the engine sets the clearance flags itself using setNodeFlags
	void					ComputeClearance(/*unsigned int clearanceValueStartBit = 0, int numClearanceValues = 0*/);
	// Does OR between flag and node data. Use COL_ flags
	void					SetNodeFlags( int xTL, int yTL, unsigned char flag );
	// Does AND ~ between flag and node data to clear all the flags in "flag". Use COL_ flags
	...
	// \param additionalSmoothingCollisionFlags | blockFlags are checked when doing the path smoothing to avoid re-adding corners
	bool					GetPath(const Vec2& start, const Vec2& end, Vec2* pPath, int maxPathPoints, int& numPathPoints, unsigned char blockFlags, bool bGetClosestPointIfBlocked = true, unsigned char additionalCostFlags = 0, unsigned char smoothingCollisionFlags = 0);
	...
	unsigned int			GetRawData_Safe(Vec2 p) const; // checks bounds
...
private:
	...
	*/
	...
	// Uses pathfinder coords (tile coords)
	eResult					GetPath( Vec2i start, Vec2i end, Vec2* pPath, int maxPathPoints, int& numPathPoints, unsigned char blockFlags, bool bGetClosestPointIfBlocked, unsigned char additionalCostFlags, unsigned char additionalSmoothingCollisionFlags );
...
private:
	int						m_width;
	int						m_height;
	unsigned char			m_blockMask;	// mask that tells when the tile is blocked or more expensive
	unsigned char*			m_nodeData{};	// DMC: keeps flags about the node
	PathNode*				m_nodemap{};

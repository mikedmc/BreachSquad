...
CStoryRoom* CMissionStory::GetNextAvailableRoom( int nGeneration, int nParentID, bool bMarkAsUsed )
{
	...
	if ( ( nGeneration < 0 ) || ( nGeneration >= (int)arrGenerations.size() ) )
		return nullptr;
	CStoryGeneration* gen = &arrGenerations[nGeneration];
	for ( int kk = 0; kk < (int)gen->arrRooms.size(); kk++ )
	{
...
{
	for ( int gg = 0; gg < (int)arrGenerations.size(); gg++ )
	{
		int nChildIdx = 0;
		CStoryGeneration* gen = &arrGenerations[gg];
		for ( int aa = 0; aa < (int)gen->arrRooms.size(); aa++ )
		{
			CStoryRoom* room = &gen->arrRooms[aa];
			room->bUsed = false;
			room->nID = gg * 100 + aa;
			// set children parents ids
			if ( gg < (int)arrGenerations.size() - 1 )
			{
				int nFakeChildren = 0;
				for ( int cc = 0; cc < room->nChildren; cc++ )
				{
					if ( nChildIdx + cc < (int)arrGenerations[gg + 1].arrRooms.size() )
					{
			...
			else if ( gg == (int)arrGenerations.size() - 1 )
			{
				room->nChildren = 0;
			}
		}
...
void CMissionStory::ClearEntriesFromGeneration( int nGeneration )
{
	...
	if ( ( nGeneration < 0 ) || ( nGeneration >= (int)arrGenerations.size() ) )
		return;
	for ( int gen = nGeneration; gen < (int)arrGenerations.size(); gen++ )
	{
		for ( int kk = 0; kk < (int)arrGenerations[gen].arrRooms.size(); kk++ )
		{

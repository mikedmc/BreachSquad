...
void CActorAIComponent::Update( CActor& act, float dTime )
{
	...
	{
		// only execute once:
		if ( AIsensor.evt.nType != K_AIEVT_DEAD )
		{
			AIsensor.evt.Set( K_AIEVT_DEAD, act.GetUID(), act._template.actorClass, act.GetPosHeart(), -1.0f, 1.0f );
			AIsensor.Reset();
			// force state decision
			CAIState* newState = act._template.AItemplate->GetHighestPriorityState( K_AIEVT_DEAD, &level.m_rand );
		...
		if ( (AItimerDecision <= 0.0f) && (!bIgnoreAIEvents) && (AIsensor.m_bEnabled) )
		{
			...
			if ( targetActor != nullptr )
			{
				...
				if ( AIsensor.pTargetedActor.GetTo() != targetActor )
				{
					...
					{
						level.AddAIEvent( K_AIEVT_SOUND_THREAT, act.GetUID(), act._template.actorClass, targetActor->GetPosHeart(), 200.0f, 0.6f );
					}
					// get ref to new target
					CSmartLink::SetLink(&AIsensor.pTargetedActor, targetActor);
				}
				// enemies overlapping
				/*
				...
				*/
				...
				if ( act.bbox.Intersects( targetActor->bbox ) )
				{
					level.AddAIEvent( K_AIEVT_TOUCH_ENEMY, targetActor->GetUID(), targetActor->_template.actorClass, targetActor->GetPosHeart(), enemyDst, 1.0f, act.GetUID() );
				}
				*/
			...
			if ( evtFinal.nType == K_AIEVT_NONE )
			{
				//nothing important, set idle tick
				evtFinal.Set( K_AIEVT_IDLE_TICK, 0, 0, Vec2( 0.0f, 0.0f ), -1.0f, 1.0f );
			}
...
CAIEvent* CActorAIComponent::GetMostImportantAIEvent( CActor& act, EAIEventType eTypeFilter /*= K_AIEVT_ANY */ )
{
	...
	for ( int kk = 0; kk < level.m_arrAIevents.GetSize(); kk++ )
	{
		...
		// ignore events generated by self
		if ( ( evt == nullptr ) || (evt->ownerUID == callerUID) || (evt->fDuration <= 0.0f) )
			continue;
		//type filter?
		if ( (eTypeFilter > K_AIEVT_NONE) && (evt->nType != eTypeFilter) )
			continue;
...
bool CActorAIComponent::SetActorAIBehaviorIdx( CActor& act, int nBehaviorIdx, bool &ret_bFinished )
{
	...
	switch ( pNewBehavior->nType )
	{
		...
		case AI_BEHAVIOR_SURPRISED:
		{
			...
			if (actor->m_AIsensorInfo.m_AIevent.nType > K_AIEVT_IDLE_TICK)
			{

...
// GetOggBuffer
//*****************************************************************************
int GetOggBuffer(WCHAR *szOggFile, BYTE **lpbData, ULONG &uDataSize, WAVEFORMATEX &wfx)
{
	// For small sounds, it'd be easier to uncompress the entire ogg
	// to a WAV structure in memory and then play it using PlaySound.
	// But that's not sensible for larger sounds. So here we'll
	// decompress it one block at a time
	OggVorbis_File vf;
	FILE *file = NULL;
	int err = OS_wfopen_s(&file, szOggFile, L"rb");	

	if(file == NULL || err != 0)
		return -1;

	int nRes = ov_open(file, &vf, NULL, 0);
	if (nRes < 0)
	{
		OS_fclose(file);
		return 1;
	...
	UINT uNumSamples = (UINT)ov_pcm_total(&vf, 0);
	UINT uNumBytes = uNumSamples * vi->channels * wfx.wBitsPerSample / 2;
	
	// allocate the data buffer
	CHAR *pbData = NULL;
	pbData = new CHAR[uNumBytes];
	if (pbData == NULL)
	{
		ov_clear(&vf);
		return 2;
	...
	int nCrtSection = 0;
	ULONG nBuffSize;
	for (nBuffSize = 0; nBuffSize < uNumBytes;)
	{
		LONG lRet = ov_read(&vf, pbData + nBuffSize, 512, 0, 2, 1, &nCrtSection);
		if (lRet < 0)
		{
			ov_clear(&vf);
			return lRet;
...
HRESULT CSoundManager::Init(HWND hWnd, DWORD dwPrimaryChannels, DWORD dwPrimaryFreq, DWORD dwPrimaryBitRate)
{
	sndOK = false;
	...
	{
		ErrorBox(K_ERR_WARNING, L"Sound->Init->SetCooperativeLevel failed (hr: %x)\n", hr);
		return hr;
	}
    ...
    if (m_pDS == NULL)
	{
		ErrorBox(K_ERR_WARNING, L"Sound->Init->m_pDS is NULL !!!\n");
		return E_FAIL;
	}
	...
	{
		ErrorBox(K_ERR_WARNING, L"Sound->Init-> CreateSoundBuffer failed (hr: %x)\n", hr);
        return hr;
	}
	...
	{
		ErrorBox(K_ERR_WARNING, L"Sound->Init->SetFormat failed (hr: %x)\n", hr);
		return hr;
	}
	...
	sndOK = true;
	LOG(L"Sounds:: Sound System Initialized OK.");
...
HRESULT CSoundManager::LoadSoundBuffers(CSound * pSound)
{
	...
	pSound->buffers = new LPDIRECTSOUNDBUFFER[pSound->buffersCnt];
...
HRESULT CSoundManager::AddSound(WCHAR* wszFile, UINT32 nID, UINT32 nGroupID, int nBuffers, bool bOnlyLoadWhilePlaying, int *retIdx)
{	
	if(retIdx)
		*retIdx = -1;
	HRESULT hr = S_OK;
	...
	StringCchCopy(sound->strFile, MAX_PATH, wszFile);
	...
	//seteaza bufferele imediat dupa ce le incarca
...
void CSoundManager::ApplySoundSettingsNow(int sndIdx, bool affectPlayingToo)
{
	...
	{
		LPDIRECTSOUNDBUFFER pDSB = snd->buffers[kk];
		if(pDSB == NULL)
			continue;
		if(!affectPlayingToo)
		{
...
void CSoundManager::SetBufferSettingsNow(int sndIdx, int nBufferIdx, float fVolume, float fFrequency, float fPanning)
{
	if (!sndOK)
		return;
	LPDIRECTSOUNDBUFFER pDSB = sounds[sndIdx]->buffers[nBufferIdx];
...
//Play
LPDIRECTSOUNDBUFFER CSoundManager::Play(int sndIdx, DWORD flags)
{
	if(!sndOK)
		return NULL;
	...
	LPDIRECTSOUNDBUFFER pDSB = sounds[sndIdx]->buffers[sounds[sndIdx]->currentBuffer];
	if(!pDSB)
	{
...
LPDIRECTSOUNDBUFFER CSoundManager::PlayID(UINT32 sndNameHash, DWORD flags /*= 0*/)
{
	if (!sndOK)
		return NULL;
...
LPDIRECTSOUNDBUFFER CSoundManager::Play(CHAR* sndID, DWORD flags)
{
	if (!sndOK)
		return NULL;
...
LPDIRECTSOUNDBUFFER CSoundManager::PlayPositionalRand2(int sndIdx1, int sndIdx2, D3DXVECTOR2 pos, DWORD flags /*= 0*/)
{
...
LPDIRECTSOUNDBUFFER CSoundManager::PlayPositional(int sndIdx, D3DXVECTOR2 pos, DWORD flags /*= 0*/)
{
	if (!sndOK)
		return NULL;
...
//PLAY fade in
LPDIRECTSOUNDBUFFER CSoundManager::PlayFadeIn(int sndIdx, float vol, DWORD flags)
{
	if(!sndOK)
		return NULL;
	if((sndIdx < 0) || (sndIdx >= sounds.GetSize()))
...
LPDIRECTSOUNDBUFFER CSoundManager::PlayFadeIn(CHAR* sndID, float vol, DWORD flags)
{
	if(!sndOK)
		return NULL;
	int sndIdx = getSndIdx(sndID);
...
LPDIRECTSOUNDBUFFER CSoundManager::PlayFadeInID(UINT32 sndID, float vol, DWORD flags)
{
	if(!sndOK)
		return NULL;
	int sndIdx = getSndIdx(sndID);
...
void CSoundManager::StopBuffer(LPDIRECTSOUNDBUFFER pDSB, bool resetSound)
{
	if (pDSB == NULL)
		return;
	//is it playing?
...
//-----------------------------------------------------------------------------
HRESULT CSoundManager::RestoreBuffer( LPDIRECTSOUNDBUFFER pDSB, BOOL* pbWasRestored )
{
	if(!sndOK)
		return S_FALSE;
